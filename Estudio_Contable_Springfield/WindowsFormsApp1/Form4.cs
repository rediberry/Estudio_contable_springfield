...
{
    public partial class Form4 : Form, IFormulario
    {
        ...
        private Boolean ValidarPeriodoLiquidacion(Empleado empleado)
        {
            bool valido = true;
            string msg = string.Empty;           
            
            if (int.Parse(comboBox3.Text)>DateTime.Now.Year || int.Parse(comboBox2.Text) > DateTime.Now.Month)
            {
                msg = "El periodo que ingresó es incorrecto.";
            }
            if (int.Parse(comboBox3.Text) == (int.Parse(empleado.FechaAlta.Year.ToString()))&& int.Parse(comboBox2.Text) < (int.Parse(empleado.FechaAlta.Month.ToString())))
            {
                msg = "El periodo que intenta liquidar es anterior al alta del empleado.";
            }
            if (int.Parse(comboBox3.Text) < (int.Parse(empleado.FechaAlta.Year.ToString())))
            {
                msg = "El periodo que intenta liquidar es anterior al alta del empleado.";
            }
            if (msg != string.Empty)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (ValidarUnicidadLiquidacion(ObtenerIdLiquidacion()) && ValidarPeriodoLiquidacion(ObtenerEmpleado()))
                    {
                        try
                        {
                            int idempleado = ObtenerIdEmpleado();
                ...
                if (listBox2.SelectedItem != null && ValidarPeriodoLiquidacion(ObtenerEmpleado()))
                {
                    try
                    {
